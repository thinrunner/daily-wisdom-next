...
// --- MediaDispatcher: platform-specific URI schemes before HTTPS fallback ---
...
export function getSpotifyDeepLink(httpsUrl) {
  if (!isValidMusicUrl(httpsUrl)) return httpsUrl;
  const trackId = extractSpotifyTrackId(httpsUrl);
  if (!trackId) return httpsUrl;
  return { uri: `spotify:track:${trackId}`, fallback: httpsUrl };
}

export function getYouTubeDeepLink(httpsUrl) {
  if (!isValidMusicUrl(httpsUrl)) return httpsUrl;
  const videoId = extractYouTubeVideoId(httpsUrl);
  if (!videoId) return httpsUrl;
  return { uri: `vnd.youtube:${videoId}`, fallback: httpsUrl };
}

export function openMusicLink(deepLink) {
  if (typeof deepLink === 'string') {
    window.open(deepLink, '_blank', 'noopener,noreferrer');
    return;
  }
  // Try URI scheme first, fall back to HTTPS after timeout
  const fallbackTimer = setTimeout(() => {
    if (document.visibilityState !== 'hidden') {
      window.open(deepLink.fallback, '_blank', 'noopener,noreferrer');
    }
  }, 1500);
  ...
  window.location.href = deepLink.uri;
}
